...
modelmartins.prototype.cadastrarpaciente = function( dataexame, susfacil, prt, paciente,leito, idade,exame, unidade,paliativo,ecf,svd, sne, avp,cvc, spict,data, callback){
	...
	this._conection.query('insert into kabanmartins set dataexame = "'+dataexame+'", susfacil = "'+susfacil+'",prt = "'+prt+'", paciente = "'+paciente+'", leito = "'+leito+'", idade = "'+idade+'", news = "" , dispositivo = "", spict="'+spict+'" , fluxo_de_o2 = "" , droga = "" , fio2 = "" ,peep = "" ,sedacao = "" , glasgow = "" , tempo = "" ,exame = "'+exame+'", unidade = "'+unidade+'", ecf="'+ecf+'", paliativo="'+paliativo+'", svd="'+svd+'", sne="'+sne+'", avp="'+avp+'", cvc="'+cvc+'", dataadmissao="'+data+'"', callback);
}
...
modelmartins.prototype.update = function(idpaciente,dataexame, susfacil,prt,paciente, leito, idade, exame,paliativo, ecf,svd, sne, avp,cvc,spict, callback){
	...
	this._conection.query('update kabanmartins set dataexame = "'+dataexame+'", susfacil = "'+susfacil+'",prt = "'+prt+'", paciente = "'+paciente+'", leito = "'+leito+'", idade = "'+idade+'" , exame = "'+exame+'", ecf="'+ecf+'", paliativo="'+paliativo+'", svd="'+svd+'", sne="'+sne+'", avp="'+avp+'", cvc="'+cvc+'", spict="'+spict+'" where idpaciente = ' + idpaciente, callback);
}

modelmartins.prototype.updatenews = function(idpaciente,news,tempo, data, fr, sat, temp, o2, sistolica, fc, alerta, callback){


	this._conection.query('update kabanmartins set  news = "'+news+'", tempo = "'+tempo+'", data = "'+data+'", fr = "'+fr+'", sat = "'+sat+'", temp = "'+temp+'", o2 = "'+o2+'", sistolica = "'+sistolica+'", fc = "'+fc+'", alerta = "'+alerta+'"  where idpaciente = ' + idpaciente, callback);
}
...
modelmartins.prototype.baixa = function(idpaciente,baixa,data, callback){
	...
	this._conection.query('update kabanmartins set   baixa = "'+baixa+'", databaixa = "'+data+'" where idpaciente = ' + idpaciente, callback);
}

modelmartins.prototype.buscarpaciente = function(unidade, callback){
	
	this._conection.query('select * from kabanmartins where unidade = "'+unidade+'" and baixa is null', callback);
}

modelmartins.prototype.historico = function(unidade, callback){
	
	this._conection.query('select * from kabanmartins where unidade = "'+unidade+'"  order by paciente asc', callback);
}

modelmartins.prototype.buscarpacienteid = function(idpaciente, unidade, callback){
	this._conection.query('select * from kabanmartins where unidade = "'+unidade+'" and idpaciente = ' + idpaciente.id, callback);
}
