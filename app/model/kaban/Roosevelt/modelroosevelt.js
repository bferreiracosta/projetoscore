...
modelroosevelt.prototype.cadastrarpaciente = function( dataexame, susfacil, prt, paciente,leito, idade,exame, unidade,paliativo,ecf,svd, sne, avp,cvc, spict,data, callback){
	...
	this._conection.query('insert into kabanroosevelt set dataexame = "'+dataexame+'", susfacil = "'+susfacil+'",prt = "'+prt+'", paciente = "'+paciente+'", leito = "'+leito+'", idade = "'+idade+'", news = "" , dispositivo = "", spict="'+spict+'" , fluxo_de_o2 = "" , droga = "" , fio2 = "" ,peep = "" ,sedacao = "" , glasgow = "" , tempo = "" ,exame = "'+exame+'", unidade = "'+unidade+'", ecf="'+ecf+'", paliativo="'+paliativo+'", svd="'+svd+'", sne="'+sne+'", avp="'+avp+'", cvc="'+cvc+'", dataadmissao="'+data+'"', callback);
}
...
modelroosevelt.prototype.update = function(idpaciente,dataexame, susfacil,prt,paciente, leito, idade, exame,paliativo, ecf,svd, sne, avp,cvc,spict, callback){
	...
	this._conection.query('update kabanroosevelt set dataexame = "'+dataexame+'", susfacil = "'+susfacil+'",prt = "'+prt+'", paciente = "'+paciente+'", leito = "'+leito+'", idade = "'+idade+'" , exame = "'+exame+'", ecf="'+ecf+'", paliativo="'+paliativo+'", svd="'+svd+'", sne="'+sne+'", avp="'+avp+'", cvc="'+cvc+'", spict="'+spict+'" where idpaciente = ' + idpaciente, callback);
}

modelroosevelt.prototype.updatenews = function(idpaciente,news,tempo, data, fr, sat, temp, o2, sistolica, fc, alerta, callback){


	this._conection.query('update kabanroosevelt set  news = "'+news+'", tempo = "'+tempo+'", data = "'+data+'", fr = "'+fr+'", sat = "'+sat+'", temp = "'+temp+'", o2 = "'+o2+'", sistolica = "'+sistolica+'", fc = "'+fc+'", alerta = "'+alerta+'"  where idpaciente = ' + idpaciente, callback);
}
...
modelroosevelt.prototype.baixa = function(idpaciente,baixa,data, callback){
	...
	this._conection.query('update kabanroosevelt set   baixa = "'+baixa+'", databaixa = "'+data+'" where idpaciente = ' + idpaciente, callback);
}

modelroosevelt.prototype.buscarpaciente = function(unidade, callback){
	
	this._conection.query('select * from kabanroosevelt where unidade = "'+unidade+'" and baixa is null', callback);
}

modelroosevelt.prototype.historico = function(unidade, callback){
	
	this._conection.query('select * from kabanroosevelt where unidade = "'+unidade+'"  order by paciente asc', callback);
}

modelroosevelt.prototype.buscarpacienteid = function(idpaciente, unidade, callback){
	this._conection.query('select * from kabanroosevelt where unidade = "'+unidade+'" and idpaciente = ' + idpaciente.id, callback);
}
